...
axs_reduce[0,1].bar(x_pos, mean['KeyGenerationTime'], width, yerr=std['KeyGenerationTime'], align='center', capsize=3, label=labels)
axs_reduce[0,2].bar(x_pos, mean['EncryptionTime'], width, yerr=std['EncryptionTime'], align='center', capsize=3, label=labels)
axs_reduce[1,0].bar(x_pos, mean['ExecutionTime'], width, yerr=std['ExecutionTime'], align='center', capsize=3, label=labels)
axs_reduce[1,1].bar(x_pos, mean['DecryptionTime'], width, yerr=std['DecryptionTime'], align='center', capsize=3, label=labels)
axs_reduce[1,2].bar(x_pos, mean['ReferenceExecutionTime'], width, yerr=std['ReferenceExecutionTime'], align='center', capsize=3, label=labels)
